...
async function viewAllReimbursements(){
   var typeS,statusValue;
    try{
         ...
         fetch("http://localhost:8080/empreimb/viewAllTickets")
...
pendingTickets();
        ...
        try{
             fetch("http://localhost:8080/empreimb/reimb/" +opt.value)
             .then(response => {return response.json()})
             .then(data =>{
...
function pendingTickets(){
    try{
    fetch("http://localhost:8080/empreimb/reimb/" +1)
             .then(response => {return response.json()})
             .then(data =>{
                 //ID/AUTHOR/AMOUNT/TYPE/SUBMIT(date)/APPROVE(date)/DESCRIPTION/STATUS/RESOLVER
                 if(data.length != 0){
...
function approval(reimbId, statusId, rowNum){
  try{
    ...
    fetch("http://localhost:8080/empreimb/update/"+statusId,{
            method:"PUT",
